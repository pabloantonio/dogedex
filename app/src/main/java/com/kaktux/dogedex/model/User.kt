package com.kaktux.dogedex.model
    ...
    companion object {
        ...
        fun setLoggedInUser(activity: Activity, user: User) {
            activity.getSharedPreferences(AUTH_PREFS, Context.MODE_PRIVATE).also {
                it.edit().putLong(ID_KEY, user.id).putString(EMAIL_KEY, user.email).putString(
                    AUTH_TOKEN_KEY,
                    user.authenticationToken,
                )
                    .apply() // este es el caso en el cual se aplican los cambios
            }
        }
        ...
        fun logOutUser(activity: Activity) {
            activity.getSharedPreferences(AUTH_PREFS, Context.MODE_PRIVATE).also {
                it.edit().clear().apply() // este es el caso en el cual se aplican los cambios
            }
        }
    }
}
